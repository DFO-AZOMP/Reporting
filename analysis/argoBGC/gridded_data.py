...
cmaps = 2*[cmo.cm.dense_r, cmo.cm.algae, cmo.cm.matter, cmo.cm.tempo, cmo.cm.amp]
...
for v, cm in zip(bgc_vars, cmaps):
    for d in depth_ranges:
        ...
        if 'BBP700' in varname:
            ix[varname] = 1000*ix[varname]
        ...
        for season, axrow in zip(['year', 'spring', 'summer', 'autumn', 'winter'], axes):
            ...
            for plot, ax in zip(['climatology', 'analysis_year', 'delta'], axrow):
                ...
                if plot == 'climatology':
                    var_mask_index = (ix.year <= clim_year) & (ix[varname].notna())
                    df = pd.DataFrame(
                        {
                            'longitude':pd.cut(ix.loc[var_mask_index, 'longitude'], xgrid, labels=xgrid[:-1]+boxsize/2), 
                            'latitude':pd.cut(ix.loc[var_mask_index, 'latitude'], ygrid, labels=ygrid[:-1]+boxsize/2), 
                            'pres':ix.loc[var_mask_index,'PRES_MAX']
                        }
                    )
                    ...
                    index = (season_index) & (ix.year <= clim_year) & (ix[varname].notna())
                    df = pd.DataFrame(
                        {
                    ...
                    param = ax.pcolormesh(X, Y, clim, cmap=cm, vmin=varinfo.loc[varname, 'vmin'], vmax=varinfo.loc[varname, 'vmax'], transform=transform)
                elif plot == 'analysis_year':
                    index = (season_index) & (ix.year == analysis_year) & (ix[varname].notna())
                    df = pd.DataFrame(
                        {
                            'longitude':pd.cut(ix.loc[index, 'longitude'], xgrid, labels=xgrid[:-1]+boxsize/2), 
                            'latitude':pd.cut(ix.loc[index, 'latitude'], ygrid, labels=ygrid[:-1]+boxsize/2), 
                    ...
                    ax.set_title(title, loc='left', fontweight='bold')
                    param = ax.pcolormesh(X, Y, grid, cmap=cm, vmin=varinfo.loc[varname]['vmin'], vmax=varinfo.loc[varname, 'vmax'], transform=transform)
                elif plot == 'delta':
                    grid = grid - clim
                    title = f'Anomaly ([{analysis_year}] - [Climatology])' if season == 'year' else ''
                    ax.set_title(title, loc='left', fontweight='bold')
                    delta = ax.pcolormesh(X, Y, grid, cmap=cmo.cm.balance, transform=transform, vmin=-varinfo.loc[varname, 'delta'], vmax=varinfo.loc[varname, 'delta'])

        cbax = fig.add_axes([0.05, 0.08, 0.59, 0.012])
        cb = plt.colorbar(param, orientation='horizontal', extend='both', cax=cbax)
        cb.set_label(varinfo.loc[varname, 'label'] + varinfo.loc[varname, 'unit'])

        cbax = fig.add_axes([0.68, 0.08, 0.28, 0.012])
        cb = plt.colorbar(delta, orientation='horizontal', extend='both', cax=cbax)
        cb.set_label('$\Delta$' + varinfo.loc[varname, 'label'] + varinfo.loc[varname, 'unit'])

        # plt.show()
        title = varinfo.loc[varname, 'long_name']
    ...
    for d in depth_ranges:
        ...
        for plot, ax in zip(['climatology', 'analysis_year', 'delta'], axes):
            ...
            if plot == 'climatology':
                ...
                index = (ix.year == analysis_year) & (ix[varname].notna())
                df = pd.DataFrame(
                    {
                        'longitude':pd.cut(ix.loc[index, 'longitude'], xgrid, labels=xgrid[:-1]+boxsize/2), 
                        'latitude':pd.cut(ix.loc[index, 'latitude'], ygrid, labels=ygrid[:-1]+boxsize/2), 
...
for v in bgc_vars:
    ...
    for plot, ax in zip(['climatology', 'analysis_year'], axes):
        ...
        if plot == 'climatology':
            ...
            df = pd.DataFrame(
                {
                    'longitude':pd.cut(ix.loc[var_mask_index, 'longitude'], xgrid, labels=xgrid[:-1]+boxsize/2), 
                    'latitude':pd.cut(ix.loc[var_mask_index, 'latitude'], ygrid, labels=ygrid[:-1]+boxsize/2), 
                    'variable':ix.loc[var_mask_index, varname]
                }
            )
            df = df.loc[df.variable.notna()]
            grid = df.groupby(['latitude', 'longitude'])['variable'].count().unstack()
            min_year = ix.loc[var_mask_index, 'year'].min()
            title = f'Climatology ({min_year}-{clim_year}'
            ax.set_title(f'{title},\n{sum(var_mask_index)} Profiles)', loc='left', fontweight='bold')
            ...
            var_mask_index = (ix.year == analysis_year) & (ix[varname].notna())
            df = pd.DataFrame(
                {
                    'longitude':pd.cut(ix.loc[var_mask_index, 'longitude'], xgrid, labels=xgrid[:-1]+boxsize/2), 
                    'latitude':pd.cut(ix.loc[var_mask_index, 'latitude'], ygrid, labels=ygrid[:-1]+boxsize/2), 
            ...
            df = df.loc[df.variable.notna()]
            grid = df.groupby(['latitude', 'longitude'])['variable'].count().unstack()
            ax.set_title(f'{analysis_year} ({sum(var_mask_index)} Profiles)', loc='left', fontweight='bold')
    ...
    sns.histplot(ix.year.loc[ix[varname].notna()]+0.5, bins=range(ix.year.min(), 2026), ax=axes[-1])
    axes[-1].yaxis.tick_right()
    axes[-1].yaxis.set_label_position("right")

    fig.suptitle(f'Profile Histogram\n\n', y=1.08)
    # plt.show()
    fig.savefig(f'figures/{analysis_year}/grid/{v}_histogram_map.png', bbox_inches='tight', dpi=350)
    plt.close(fig)
...
for v in bgc_vars:
    ...
    # plt.show()
    fig.savefig(f'figures/{analysis_year}/grid/{v}_masks_map.png', bbox_inches='tight', dpi=350)
    plt.close(fig)
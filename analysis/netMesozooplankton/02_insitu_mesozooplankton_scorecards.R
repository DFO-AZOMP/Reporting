...
dir.create(paste0("analysis/netMesozooplankton/figures/",report_year,"/"), recursive=TRUE, showWarnings=FALSE)
...
variable_str <- c("Calanus finmarchicus", "%PDI", "Calanus glacialis", "Calanus hyperboreus", "Pseudocalanus", "Oithona", "Euphausiid", "Amphipoda")
# note the space before %PDI, this is necessary to make ggtext::element_markdown work
variable_lbl <- ifelse(variable_str=="%PDI"," %PDI",paste0("*",variable_str,"*"))
meansd_format <- c("%.0e","%.1f",rep("%.0e",6))

output_file <- paste0("analysis/netMesozooplankton/figures/",report_year,"/",gsub(".txt","",basename(input_file)),"_Scorecard_",paste0(range(years),collapse="-"),"_ref",paste0(range(ref_years),collapse="-"),".png")
...
gt <- gt + theme(strip.text=ggtext::element_markdown())

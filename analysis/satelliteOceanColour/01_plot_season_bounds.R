...
output_file <- "analysis/satelliteOceanColour/figures/labrador_sea_polygon_season_bounds.png"
...
p <- plots
p[2:length(p)] <- lapply(p[2:length(p)], function(x) {x+theme(axis.text.y=element_blank())})
...
ggsave(filename=gsub(".png","_vertical.png",output_file), plot=p, height=1400, width=800, dpi=150, units="px")

...
for v in phy_vars:
    ...
    for ax, dr in zip(axes, depth_ranges):
        ...
        sns.scatterplot(data=df, x='dayofyear', y=varname, hue='year', ax=ax, legend=False, linewidth=0.1, s=0.8, zorder=2)
        ax.set_ylabel(v)
        ax.set_title(f'{dr[0]} - {dr[1]} dbar', loc='left')
    
    fig.tight_layout()
    fig.set_size_inches(fig.get_figwidth()/3, fig.get_figheight())
    fig.savefig(f'../Figures/argo/{v}_yearday.png', dpi=350, bbox_inches='tight')
    plt.close(fig)